...
async function saferCovid(details) {
  ...
  async function fillAndVerify(selector, value) {
    ...
    if (fieldValue === value) {
      ...
      console.error(
        `Error: Field ${selector} not filled correctly. Expected: ${value}, but got: ${fieldValue}`
      );
      return false;
    }
    return true;
  ...
  if (allFieldsCorrect) {
    ...
    if (selectedState !== details.state) {
      console.error(
        `Error: State not selected correctly. Expected: ${details.state}, but got: ${selectedState}`
      );
      allFieldsCorrect = false;
    }
    console.log(
      `Field ${selectors.state} correctly filled with: ${details.state}`
    );
  }

  if (allFieldsCorrect) {
    console.log(
      `All fields filled correctly. Submitting form for ${details.name}...`
    );
    await page.click('.standard-button');
    console.log(
      `Waiting for navigation after form submission for ${details.name}...`
    );
    ...
    console.log(
      `Form not submitted due to incorrect field values for ${details.name}.`
    );
  }
